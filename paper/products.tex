...
% - HOGG: Matrix cookbook, Roweis notes
% - HOGG: Refer to Luger, F-M, Hogg RNAAS? https://arxiv.org/pdf/1710.11136.pdf
% - APW: We mention SPEED of computation in the abstract; we should say that
%   again in the introduction and parts of the conversation below it. Refer to
%   The Joker appropriately.
% - audit for and remove all TODO, HOGG, DWH, APW, BL, CITE from the text.
...
% - put punctuation after equations as necessary, with "\hoggquad,". Use \hoggquad
...
\newcommand{\apjs}{ApJS} % Astrophysical Journal Supplements
...
\apw{I don't think this line needs to appear in the abstract. For those who understand the factorization, it doesn't add anything (we mention later in the text). For those who don't know what we're talking about, this could be confusing:} This magic also holds in many Gaussian-process contexts.
...
Collider} events affected by the Higgs boson \citep[see, for example,
][]{TODO}\todo{HOGG}{CITE here}:
...
variation of a star in response to an faint, orbiting companion \citep[see, for an example in our own work, ][]{Price-Whelan:2017, Price-Whelan:2020}\footnote{That project and papers would have been
  impossible without the speed-ups provided by the expressions derived in this
  \documentname.}, where the expectation of the
...
parameters too, of course.\apw{The jump from this par to the next par could use smoothing, or an intermediate section heading?}
...
data-driven model. A full discussion of \acronym{GP}s is out of scope for this
...
\begin{equation}
\mbox{\small likelihood:} \hoggquad p(\vy\given\vtheta,\bP) \hoggquad.
\end{equation}
...
function
\begin{equation}
\mbox{\small marginalized likelihood:} \hoggquad p(\vy\given\bP) \hoggquad.
\end{equation}
That's good, but the marginalization comes at a cost:
We have to choose a prior
\begin{equation}
\mbox{\small prior on nuisance parameters:} \hoggquad p(\vtheta\given\bP) \hoggquad.
\end{equation}
...
in the pedagogical literature\footnote{For example, \todo{HOGG}{CITE}, who claim that the
...
\begin{equation}
\mbox{\small posterior for nuisance parameters:} \hoggquad p(\vtheta\given\vy,\bP) \hoggquad.
\end{equation}
...
the matrix determinant lemma \citep[for example,][]{Wiki:MDL, Harville:2011}
...
all the quadratic forms, plus some use of the matrix inversion lemma \citep[also
called the Woodbury matrix identity; for example,][]{wiki:MIL, Harville:2011},
...
\end{equation}
\begin{equation}
\tH\equiv\tV\inv \hoggquad;\hoggquad
\veta\equiv\tV\inv\cdot\vm \hoggquad;\hoggquad
\xi\equiv\ln||2\pi\,\tV||+\veta^T\cdot\tV\cdot\veta \hoggquad.
\end{equation}
...
(although we don't think it often should be\footnote{\todo{HOGG}{WRITE STUFF
ABOUT WHY YOU WANT TO PUBLISH LFs and marginalized LFs.}}); these expressions
deliver the posterior straightforwardly too. \apw{Didn't you just talk about
this above? Or do you mean the full posterior pdf, not just the conditional?}
The variance tensor $\tA$ is the posterior variance in the parameter space.
It is strictly non-larger \apw{smaller?} (in eigenvalues or determinant) than either
...
It is strictly non-smaller \apw{larger?} than both the data noise variance $\tC$ and the
...
$\tC\inv$---you might have to \emph{think about how you implement the
  linear algebra operations to be zero-safe}\footnote{\todo{HOGG}{WRITE SOMETHING
...
\begin{equation}
  \mbox{\small initialize:}\hoggquad
  \tA_0\inv = \tLambda\inv \hoggquad;\hoggquad \va_0 = \vmu \hoggquad;\hoggquad \vx_0 = \tLambda\inv\cdot\vmu
\end{equation}
\begin{align}
  \mbox{\small iterate:}\hoggquad
...
mean $\va$ and variance $\tA$ \emph{do not depend} on the order!\footnote{\todo{HOGG}{say things about the consistency of Bayesian reasoning and Cox theorems; CITE Jaynes.}}
...
\todo{APW}{Connect this to centroiding peaks in spectroscopy or whatever - give it negative curvature}
...
The table shown in Figure~\ref{fig:data1} contains four data points, $(x_i, y_i,
\sigma_{y_i})$.
...
\begin{table}[t!]
  ...
  \begin{center}
    \begin{tabular}{c c}
      ...
      \adjustbox{valign=t}{\includegraphics[width=0.7\textwidth]{exercise1.pdf}} \\
    \end{tabular}

    \caption{\textbf{Figure 1}:
    Left: Simulated data used in Exercise 1
    (\href{https://raw.githubusercontent.com/davidwhogg/GaussianProductRefactor/master/notebooks/data1.csv}{link
    to data}). These data were generated with the true parameter values
    $(\alpha, \beta, \gamma) = (3.21, 2.44, 14.82)$.
    Right: The solution to Exercise 1. The MAP parameter values were found using
    \equationname~\ref{eq:a} with the data shown in the included table.
    \label{fig:data1}}
  \end{center}
\end{table}
...
\todo{APW}{Set up this problem by stating some explicit Astro problems with this structure. Mention asteroseismology, if summing over frequency}
...
\begin{align}
  ...
  (\mu_\gamma, \sigma_\gamma) &= (0, 10) \hoggquad,
\end{align}
compute the MAP parameter values $\va\T = (\alpha_{\rm MAP}, \beta_{\rm MAP},
\gamma_{\rm MAP})$.

...
% - APW: Feel free to ignore: Can we get the date to follow the author directly
...
\end{equation}
and we assume we have Gaussian prior pdfs on all of the linear parameters
$(\alpha, \beta, \gamma)$,
\begin{align}
...
\sigma_{y_i})$, generated using this quadratic model.
...
\begin{table}[t!]
  ...
  \begin{center}
    ...
    \textsl{Right:} The solution to Exercise 1. The data points (black markers)
    show the data included in the table (left). The MAP parameter values were
...
The table shown in Figure~\ref{fig:data2} contains four data points, $(x_i, y_i,
\sigma_{y_i})$, generated with this sinusoid model.
Assuming values for the prior means, $\vmu$, and prior variance tensor,
$\tLambda$,
\begin{align}
...
between (0.1, 100).
Plot $\ln p(\vy\given\omega) + \ln p(\vy\given\omega)$ as a function of this
frequency grid.
...
\begin{table}[t!]
  ...
  \begin{center}
    \begin{tabular}{c c}
      ...
      % \adjustbox{valign=t}{\includegraphics[width=0.7\textwidth]{exercise2a.pdf}} \\
    \end{tabular}
    \caption{\textbf{Figure 2}:
    \textsl{Left:} Simulated data used in Exercise 2
    (\href{https://raw.githubusercontent.com/davidwhogg/GaussianProductRefactor/master/notebooks/data2.csv}{link
    to data}). These data were generated with the true parameter values
    $(\alpha, \beta, \gamma, \omega) = (3.21, 2.44, 13.6, 1.27)$.
    \textsl{Right:} The solution to Exercise 2.
    \label{fig:data2}}
  \end{center}
\end{table}

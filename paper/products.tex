...
\newcommand{\bl}[1]{\textcolor{DarkOrchid}{[BL says: #1]}}
...
model (which makes generating samples or computing integrals far easier), and
(2) to reduce the dimensionality of your model (i.e., by enabling analytic
marginalizations of your linear parameters).
...
examples: The examples are written such that the problems are set up first, and
the solutions are given below.


\subsection{Exercise 1: A fully linear model}

\textbf{Problem:} We observe a set of $N>2$ data $(x_i, y_i)$ (indexed by $i$)
with known, Gaussian uncertainties in $y$, $\sigma_{y_i}$, and no uncertainty in
$x$.
Our parametric model for this data is a straight line,
\begin{equation}
  % APW: I used greek here because a, b are used in expressions above...
  f(x \,;\, \alpha, \beta) = \alpha\,x + \beta
\end{equation}
and we assume we have Gaussian prior pdfs on the parameters $(\alpha, \beta)$,
\begin{align}
  p(\alpha) &= \normal(\alpha \given \mu_\alpha, \sigma_\alpha)\\
  p(\beta) &= \normal(\beta \given \mu_\beta, \sigma_\beta)
  \quad .
\end{align}
Derive the form of the posterior pdf and the value of the MAP parameter vector.

Table~\ref{tbl:data1} contains 5 data points, $(x_i, y_i, \sigma_{y_i})$.
Assuming
\begin{align}
  (\mu_\alpha, \sigma_\alpha) &= (1, 5)\\
  (\mu_\beta, \sigma_\beta) &= (3, 2) \quad ,
\end{align}
compute the MAP parameter values $(\alpha_{\rm MAP}, \beta_{\rm MAP})$;
...
% Generated with Exercise1.ipynb
\begin{table}[h!]
  \begin{center}
    \label{tbl:data1}
    \begin{tabular}{c|c|c}
      ...
      $3.1$ & $1.9$ & $13.8$ \\
    \end{tabular}
    \caption{Data used in Exercise 1. \href{https://raw.githubusercontent.com/davidwhogg/GaussianProductRefactor/master/notebooks/data1.csv}{$\boxplus$}}\todo{Replace url with Zenodo link.}
  \end{center}
\end{table}
...
% FML as a function of nonlinear parameters
% MAP value and posterior for the linear parameters

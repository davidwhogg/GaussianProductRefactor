...
% - HOGG: audit terms defined in quotation marks
% - APW: audit non-linear's
...
%   in the introduction and parts of the conversation below it. Maybe even refer to
...
\newcommand{\bP}{\ensuremath{\textrm{\ding{80}}}} % THIS IS HERE TO ANNOY DWH
...
It is common in physics and astronomy that likelihood functions
...
events
affected by the Higgs boson (see, for example, HOGG CITE);
...
variation of a star in response to an faint, orbiting companion (see, for
example, APW CITE JOKER; this is also a
problem we will use as an example below), where the expectation of the
...
That is, there are many data vectors $y_i$, each of which has size or length $N_i$,
and each of which has an expectation 
set linearly by the parameters $v\theta$ but through a different design matrix $\mM_i$.
Provided that the different data vectors $y_i$ are independently ``observed'' (that
